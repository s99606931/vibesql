...
describe("[히스토리 로드] GET /api/history", () => {
  ...
  it("초기 상태 → 빈 배열", async () => {
    ...
    const res = (await GET(new Request("http://localhost/api/history"))) as unknown as ApiResponse;
...
describe("[전체 플로우] 쿼리 실행 → 히스토리 기록 → 별표 → 재실행", () => {
  ...
  it("완전한 히스토리 사용자 여정", async () => {
    ...
    const listRes = (await histGet(new Request("http://localhost/api/history"))) as unknown as ApiResponse;
